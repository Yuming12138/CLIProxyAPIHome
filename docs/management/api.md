...
| `capabilities.quota_reset_credit_consume` | boolean | Whether guarded Home-owned Codex reset-credit consumption is available. |
...
Consumes the current earliest-expiring Codex reset credit through the Home-owned credential. This is a mutating operation and is available only when `capabilities.quota_reset_credit_consume=true`. It never returns the provider-issued credit identifier or any credential material.
...
| Field | Type | Description |
| --- | --- | --- |
| `redeem_request_id` | UUID string | Required idempotency key. A caller must reuse the exact same value after a timeout or ambiguous network failure. |
| `expected_expires_at` | RFC3339 timestamp | Required optimistic guard. It must equal the current earliest available credit expiry returned by the detail endpoint. |

Home accepts the operation only for an enabled Codex OAuth/file credential with a fresh, complete, current-version `active_probe` or `mixed` snapshot, a positive available count, and a non-expired earliest credit matching `expected_expires_at`. If a newer recollection changes the earliest credit or its expiry first, the old request is rejected without contacting the consume endpoint. Automation must also stop calling after a newer quota snapshot shows an official or manual refill; the endpoint cannot predict an upstream reset that Home has not collected yet.

A database-backed per-credential lease serializes concurrent Home instances. The same successfully completed `redeem_request_id` is returned as an idempotent replay without another upstream request. A different request ID cannot be used after an attempt until a newer authoritative snapshot has been stored. Upstream `401` or `403` triggers one forced credential refresh and one retry. After success, Home immediately marks the source snapshot stale and queues a forced Codex recollection.
...
Validation failures return `400`; a missing credential or runtime without the consumer returns `404`; disabled credentials, stale/incomplete snapshots, changed expiry, unavailable credits, concurrent claims, and requests that require a newer recollection return `409`; upstream failures return `502`; credential refresh or completion-state persistence failures return `503`. `RESET_CREDIT_CONSUME_IN_PROGRESS` also includes `Retry-After: 30`. All errors use the quota error envelope and do not include upstream response bodies, tokens, or provider credit IDs.

### POST `/quota/collect`

Starts an asynchronous on-demand quota collection round and returns how many credentials were accepted into the local collector queue. On-demand jobs share the same process-wide provider concurrency controls as scheduled collection and are deduplicated per credential while queued or running.
...
`accepted` counts eligible credentials newly queued by this request. Disabled credentials, credentials in non-quota execution cooldown, collector-unsupported credentials, and credentials already queued or running locally are skipped. A quota-exhaustion cooldown remains eligible so the request can confirm an early operator reset. When a queued job reaches a concurrency slot it force-claims the DB probe lease: an active lease is still respected, but snapshot freshness, `next_probe_at`, and quota retry backoff do not suppress the requested attempt. The collection round runs in the background; read updated snapshots through `GET /quota/credentials` after it finishes. When the runtime has no quota collector wired, the route returns `404` with `QUOTA_RECOLLECT_UNSUPPORTED` and `capabilities.quota_recollect` is `false`.
...
| `disable-cooling` | boolean | Globally disables downstream quota cooldown scheduling. Home forces this to `true` for CPA nodes, while Home's authoritative central scheduler still applies quota cooldowns; a credential-level `disable-cooling=true` override can still opt that credential out. |
