...
var modelsURLs = []string{
	"https://raw.githubusercontent.com/Yuming12138/newapi-cmsg/refs/heads/dev/cmsg/cliproxyapi/internal/registry/models/models.json",
	...
	for _, url := range modelsURLs {
		...
		if resp.StatusCode != 200 {
			if errClose := resp.Body.Close(); errClose != nil {
				log.Errorf("models fetch: response body close error: %v", errClose)
			}
			cancel()
		...
		if errRead != nil {
			log.Debugf("models fetch read error from %s: %v", url, errRead)
			continue
		}
